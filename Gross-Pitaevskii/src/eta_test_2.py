...
class GrossPitaevskiiPINN(nn.Module):
    ...
    def pde_loss(self, inputs, predictions, gamma, potential_type="harmonic", precomputed_potential=None):
        ...
        # Calculate chemical potential exactly as per Algorithm 2
        kinetic = -0.5 * u_xx
        potential = V * u
    ...
    for mode in modes:
        ...
        for gamma in gamma_values:
            ...
            for epoch in range(epochs):
                ...
                # Calculate losses
                pde_loss, _, lambda_pde, full_u = model.pde_loss(X_tensor, u_pred, gamma, potential_type)
...
def plot_wavefunction_densities(models_by_mode, X_test, gamma_values, modes, save_dir="plots"):
    """
    Plot wavefunction densities for different modes and gamma values to reproduce Figure 11.
    """
    os.makedirs(save_dir, exist_ok=True)

    # Generate combined figure with all modes (similar to Fig 11)
    fig, axes = plt.subplots(2, 2, figsize=(12, 10))
    axes = axes.flatten()

    # Also generate individual figures for each mode
    for i, mode in enumerate(modes):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, f"mode_{mode}_densities.png"), dpi=300)
        plt.close()

        # Configure axes for combined figure
    ...
    fig.suptitle("Wavefunction Densities (Figure 11)", fontsize=16)
    fig.tight_layout(rect=[0, 0, 1, 0.96])
    fig.savefig(os.path.join(save_dir, "figure11_all_modes.png"), dpi=300)
    plt.close(fig)
...
def plot_mu_vs_gamma(mu_table, modes, save_dir="plots"):
    ...
    plt.title("Chemical Potential vs. Interaction Strength", fontsize=14)
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "mu_vs_gamma.png"), dpi=300)
    plt.close()
...
if __name__ == "__main__":
    ...
    modes = [0, 1, 2, 3]  # Modes to reproduce Figure 11
