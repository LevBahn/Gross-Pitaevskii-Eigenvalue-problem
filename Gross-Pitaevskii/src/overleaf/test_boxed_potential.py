...
class GrossPitaevskiiPINN(nn.Module):
    ...
    def pde_loss(self, inputs, predictions, gamma, p, potential_type="box", precomputed_potential=None):
        ...
        # Modified interaction term for mode 2 to amplify the nonlinear effect
        if self.mode == 2 and gamma > 0:
            # Use a more powerful nonlinear effect for mode 2
            interaction = gamma * torch.abs(u) ** 2 * u
        else:
...
def train_gpe_model(gamma_values, modes, L, p, X_train, layers, epochs,
    ...
    for mode in modes:
        ...
        if mode == 2:
            # For mode 2, use a larger network
            current_layers = [1, 64, 128, 64, 1]
            mode_lr = 5e-4  # Smaller learning rate for mode 2
            mode_epochs = min(epochs * 2, 5000)  # More epochs for mode 2
        else:
        ...
        for gamma in gamma_values:
            ...
            model = GrossPitaevskiiPINN(current_layers, mode=mode, gamma=gamma, L=L).to(device)

            # If this isn't the first gamma value, initialize with previous model's weights
            if prev_model is not None and prev_model.layers == model.layers:
                ...
                if mode == 2:
                    ...
                    # Use the advanced initialization for other modes
                    model.apply(lambda m: advanced_initialization(m, mode))

            # Adam optimizer with mode-specific learning rate
...
if __name__ == "__main__":
    # Setup parameters
    L = 1.0  # Length of the box (0 to L)
    N_f = 4000  # Number of collocation points
    epochs = 2001  # Base epochs for training
    layers = [1, 64, 64, 64, 1]  # Base neural network architecture
    ...
    # Create a specific directory for box potential with improved mode 2
    save_dir = "plots_box_figure5_improved_mode2"
    os.makedirs(save_dir, exist_ok=True)
