...
class GrossPitaevskiiPINN(nn.Module):
    ...
    def __init__(self, layers, hbar=1.0, m=1.0, mode=0, gamma=1.0, L=1.0, use_residual=True):
        ...
        use_residual : bool, optional
            Whether to use residual blocks (default is True).
        """
        super().__init__()
        self.layers = layers
        self.use_residual = use_residual  # Set this attribute BEFORE calling build_network
    ...
    def energy_eigenvalue(self, n, potential_type="box"):
        """
        Compute the energy eigenvalue for mode n.
        """
        if potential_type == "box":
            n_actual = n + 1  # Convert mode to quantum number
            return (n_actual ** 2 * np.pi ** 2) / (2 * self.L ** 2)
        elif potential_type == "harmonic":
            # For harmonic oscillator, E_n = (n + 0.5)ℏω, with ℏ = ω = 1
            return n + 0.5
        else:
    ...
    def pde_loss(self, inputs, predictions, gamma, p, potential_type="box", precomputed_potential=None):
        ...
        # For γ = 0.0, we can add a term to push towards the correct energy
        if gamma == 0.0:
            # Get the analytical energy value based on potential type
            exact_energy = self.energy_eigenvalue(self.mode, potential_type)

            # Add a penalty term to push towards the correct energy
            energy_penalty = 5.0 * (lambda_pde - exact_energy) ** 2
        else:
            energy_penalty = 0.0
        ...
        # PDE loss (mean squared residual) + energy penalty
        pde_loss = torch.mean(pde_residual ** 2) + energy_penalty
...
def train_gpe_model(gamma_values, modes, p, X_train, lb, ub, layers, epochs,
    ...
    This version handles different architectures between gamma values properly.
    """
    ...
    for mode in modes:
        ...
        # For each gamma value, we train from scratch rather than loading previous model
        # This avoids architecture mismatch issues
        for gamma in gamma_values:
            if verbose:
                print(
                    f"\nTraining for γ = {gamma:.2f}, mode = {mode}, nonlinearity p = {p}, potential = {potential_type}")

            # Determine architecture based on mode and gamma
            use_residual = (mode >= 2 and gamma > 0)

            # Increase network width/depth based on mode number
            # if mode >= 4:
            #     # Deeper network with wider hidden layers
            #     actual_layers = [1, 256, 256, 256, 256, 1]
            if mode >= 2:
                actual_layers = [1, 128, 128, 128, 128, 1]
            else:
                actual_layers = layers

            # Initialize model for this mode and gamma
            model = GrossPitaevskiiPINN(actual_layers, mode=mode, gamma=gamma, use_residual=use_residual).to(device)

            # Use the advanced initialization that considers mode number
            model.apply(lambda m: advanced_initialization(m, mode))

            # Adam optimizer with smaller learning rate for nonlinear cases
            if mode >= 2 and gamma > 0:
                actual_lr = lr * 0.5  # Reduce learning rate for challenging cases
            else:
                actual_lr = lr
            ...
            #     optimizer, mode='min', factor=0.2, patience=50, min_lr=1e-6,
            #     threshold=1e-4, verbose=verbose
            # )
            ...
            for epoch in range(epochs):
                ...
                # For γ = 0.0 and the box potential, we know the exact energy
                energy_loss = 0.0
                ...
                # Adjust weights based on mode and gamma
                if mode >= 2 and gamma > 0:
                    # Increase PDE loss weight for challenging cases
                    total_loss = pde_loss + 10.0 * boundary_loss + 20.0 * norm_loss + 5.0 * sym_loss
                else:
                    # Standard weights for simpler cases
                    total_loss = pde_loss + 10.0 * boundary_loss + 20.0 * norm_loss + 5.0 * sym_loss

                # Backpropagate
                total_loss.backward()

                # Stronger gradient clipping for higher modes
                clip_norm = 0.1 if (mode >= 4 and gamma > 0) else (0.5 if mode >= 2 and gamma > 0 else 1.0)
                ...
                if epoch % 100 == 0:
                    ...
                    if verbose and epoch % 500 == 0:
                        loss_str = (
                            f"Epoch {epoch}, Total Loss: {total_loss.item():.6f}, PDE Loss: {pde_loss.item():.6f}, "
                            f"Boundary Loss: {boundary_loss.item():.6f}, Norm Loss: {norm_loss.item():.6f}, "
                            f"Sym Loss: {sym_loss.item():.6f}")

                        if energy_loss > 0:
                            loss_str += f", Energy Loss: {energy_loss.item():.6f}"
...
def plot_wavefunction(models_by_mode, X_test, gamma_values, modes, p, lb, ub, potential_type="box", save_dir="tmp"):
    """
...
def plot_mu_vs_gamma(mu_table, modes, p, potential_type="box", save_dir="tmp"):
    """
...
if __name__ == "__main__":
    ...
    gamma_values = [0.0, 10.0, 20.0, 40.0, 70.0, 100.0]
    ...
    for p in nonlinearity_powers:
        ...
        for potential_type in all_potentials:
            ...
            p_save_dir = f"matthew_test_p{p}_{potential_type}"
            ...
            if potential_type == 'box':
                ...
                for mode in modes:
                    L = ub
                    energy = ((mode + 1) ** 2 * np.pi ** 2) / (2 * L ** 2)  # Box is assumed to be from lower bound to upper bound 1
