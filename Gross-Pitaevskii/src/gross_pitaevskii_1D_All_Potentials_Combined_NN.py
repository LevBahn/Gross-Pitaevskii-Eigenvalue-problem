...
class HarmonicPotentialNN(nn.Module):
    ...
    def __init__(self, layers, hbar=1.0, m=1.0, gamma=1.0, mode=0):
        super().__init__()
        self.layers = layers
    ...
    def forward(self, x, lambda_pde, potential):
        """
        ...
        """
        if self.gamma == 0:
            return self.weighted_hermite(x, self.mode)
        elif self.mode == 0:
    ...
    def boundary_loss(self, inputs, boundary_points, mode):
        """
        ...
        """
        # u = u_0 + N
        u_pred = self.weighted_hermite(self, inputs, mode) + self.forward(boundary_points)
    ...
    def riesz_loss(self, predictions, inputs, eta, mode, potential_type, precomputed_potential=None):
        ...
        u = self.weighted_hermite(self, inputs, mode) + predictions
...
class GrossPitaevskiiPINN(nn.Module):
    ...
    def __init__(self, layers, hbar=1.0, m=1.0, g=100.0):
        ...
        self.harmonic_nn = HarmonicPotentialNN()
    ...
    def compute_harmonic_solution(self, x, lambda_pde, potential):
        """
        Use the harmonic potential neural network to predict the harmonic solution.
        """
        return self.harmonic_nn(x, lambda_pde, potential)
...
def train_and_save_pinn(X, N_u, N_f, layers, eta, epochs, lb, ub, weights, model_save_path, potential_type):
    ...
    # Train the model
    model, loss_history, harmonic_predictions = train_pinn(X, N_u=N_u, N_f=N_f, layers=layers, eta=eta, epochs=epochs, lb=lb, ub=ub,
...
def predict_and_plot(models, etas, X_test, harmonic_preds, save_path='plots/predicted_solutions.png',
    ...
    for model, eta in zip(models, etas):
        ...
        potential = 0.5 * harmonic_preds[etas.index(eta)] ** 2

        # Calculate lambda from the lowest energy ground state
