        ...
        for gamma in fine_gamma_curriculum:
            ...
            for epoch in range(epochs):
                ...
                if verbose and epoch % 100 == 0:
                    # Compute chemical potential for monitoring
                    # This needs to be done with fresh tensors to avoid autograd errors
                    with torch.no_grad():  # Disable gradients for evaluation
                        X_eval = torch.tensor(X_train, dtype=torch.float32).to(device)
                        # Get normalized wavefunction using forward pass
                    ...
                    # Now calculate chemical potential using detached tensors
                    X_mu = torch.tensor(X_train, dtype=torch.float32, requires_grad=True).to(device)
                    mu = model.chemical_potential(X_mu, gamma, potential_type, precomputed_potential=None,
                                                  u_normalized=None)

                    # Print status
                    print(f"Epoch {epoch}, Energy: {energy.item():.6f}, μ: {mu.item():.6f}")

                    # For higher modes, print constraint values
                    if mode > 0 and epoch % 500 == 0:
                        print(f"  Node constraint: {node_loss.item():.6f}, Boundary: {boundary_loss.item():.6f}")
                        if mode >= 3:
                            print(f"  Frequency constraint: {freq_loss.item():.6f}")
            ...
            # to avoid autograd errors during evaluation
            with torch.no_grad():
            ...
            if save_model:
                mu_logs.append((gamma, mu_final.item()))
...
def plot_wavefunction_densities(models_by_mode, X_test, gamma_values, modes, save_dir="plots"):
    ...
    gamma_values : list of float
        Interaction strength values to plot
    modes : list of int
