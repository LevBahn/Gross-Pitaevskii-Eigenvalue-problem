            ...
            for epoch in range(epochs):
                ...
                if epoch % 100 == 0:
                    ...
                    if verbose and epoch % 500 == 0:
                        print(
                            f"Epoch {epoch}, {"Total Loss"} : {total_loss.item():.6f}, {loss_type}: {physics_loss.item():.6f}, "
                            f"Constraints: {constraint_loss:.6f}, μ: {lambda_value.item():.4f}")
...
def plot_wavefunction(models_by_mode, X_test, gamma_values, modes, save_dir="Gross-Pitaevskii/src/final/refine/test"):
    """
...
def plot_combined_grid(models_by_mode, X_test, gamma_values, modes, save_dir="Gross-Pitaevskii/src/final/refine/test"):
    """
...
def plot_combined_loss_history(training_history, modes, gamma_values, epochs,
...
def plot_all_modes_gamma_loss(training_history, modes, gamma_values, epochs,
                              save_dir="Gross-Pitaevskii/src/final/refine/test"):
    """
...
def plot_improved_loss_visualization(training_history, modes, gamma_values, epochs,
                                     save_dir="Gross-Pitaevskii/src/final/refine/test"):
    """
...
if __name__ == "__main__":
    ...
    X_test = np.linspace(lb, ub, 1000).reshape(-1, 1)  # Higher resolution for plotting_loss
    ...
    p_save_dir = f"plots_p_harmonic_bahn"
    ...
    plot_wavefunction(models_by_mode, X_test, gamma_values, modes, p_save_dir)

    # Plot μ vs γ for all modes
    print("Generating chemical potential vs. gamma plot...")
    plot_mu_vs_gamma(mu_table, modes, p_save_dir)
    ...
    plot_all_modes_gamma_loss(training_history, modes, gamma_values, epochs, p_save_dir)