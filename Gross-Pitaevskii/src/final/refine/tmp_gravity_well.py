...
class GrossPitaevskiiPINN(nn.Module):
    ...
    def airy_solution(self, x, n):
        """
        Linear eigenfunction: Î¨_n(x) = Ai(x + x_n)
        """
        from scipy.special import airy, ai_zeros
        # Convert to numpy for calculation
        x_np = x.detach().cpu().numpy()
        # Get n-th zero of Airy function
        x_n = ai_zeros(n + 1)[0][n]
        # Eigenfunction: Î¨_n(x) = Ai(x + x_n)
        psi = airy(x_np + x_n)[0]
...
def train_gpe_model(gamma_values, modes, p, X_train, lb, ub, layers,
    ...
    for mode in modes:
        ...
        for gamma in gamma_values:
            if verbose:
                print(f"\nTraining for Î³ = {gamma:.2f}, mode = {mode}, nonlinearity p = {p}")
            ...
            if prev_model is not None:
                ...
                if verbose:
                    ...
                    print(f"  Expected Î¼ for mode {mode}: {expected_mu:.4f}")
            else:
            ...
            for epoch in range(epochs):
                ...
                if epoch % 100 == 0:
                    ...
                    if verbose and epoch % 500 == 0:
                        print(f"Epoch {epoch}, Î¼: {lambda_value.item():.4f}")
...
def plot_improved_loss_visualization(training_history, modes, gamma_values, epochs, p, potential_type,
    ...
    for i, mode in enumerate(modes):
        for gamma in [0.0]:  # Focus on Î³=0 for clarity
...
def train_regular_pinn(mode, gamma, p, X_train, lb, ub, layers, epochs, lr=1e-3, tol=1e-5, perturb_const=0.01,
    ...
    for epoch in range(epochs):
        ...
        if current_loss <= tol or patience_counter >= patience:
            if verbose:
                print(f"Vanilla PINN - Mode {mode}, Î³={gamma}, stopped at epoch {epoch}, loss: {current_loss:.6f}")
            break
        if verbose and epoch % 500 == 0:
            print(f"Vanilla PINN - Mode {mode}, Î³={gamma}, Epoch {epoch}, Loss: {current_loss:.6f}")
...
def train_or_load_regular_pinns(modes, gamma_values, p, X_train, lb, ub, layers, epochs,
    ...
    if not force_retrain:
        ...
        if (regular_models is not None and
            ...
            print("âœ“ Found matching cached Vanilla PINN models!")
            return regular_models, regular_constants
        else:
            print("âœ— Cached Vanilla PINN models don't match current parameters.")
    ...
    for mode in modes:
        regular_models[mode] = {}
        for gamma in gamma_values:
            print(f"Training Vanilla PINN - Mode {mode}, Î³={gamma}")
...
def train_or_load_curriculum_pinns(modes, gamma_values, p, X_train, lb, ub, layers, epochs,
    ...
    if not force_retrain:
        ...
        if (curriculum_models is not None and
                ...
                set(cached_gammas) == set(gamma_values) and
                cached_p == p and cached_potential == potential_type):
            print("âœ“ Found matching cached Curriculum PINN models!")
            return curriculum_models, curriculum_constants
        else:
            print("âœ— Cached Curriculum PINN models don't match current parameters.")
    ...
    for mode in modes:
        for gamma in comparison_gammas:
            if mode in regular_models and gamma in regular_models[mode]:
                ...
                print(f"Vanilla PINN - Mode {mode}, Î³={gamma} -> Abs Error: {abs_err:.2e}, Rel Error: {rel_err:.3f}%")
    ...
    for mode in modes:
        for gamma in comparison_gammas:
            if mode in curriculum_models and gamma in curriculum_models[mode]:
                ...
                print(f"Curriculum - Mode {mode}, Î³={gamma} -> Abs Error: {abs_err:.2e}, Rel Error: {rel_err:.3f}%")
    # Evaluate PL-PINNs
    ...
    for mode in modes:
        for gamma in comparison_gammas:
            if mode in pl_pinn_models and gamma in pl_pinn_models[mode]:
                ...
                print(f"PL-PINN - Mode {mode}, Î³={gamma} -> Abs Error: {abs_err:.2e}, Rel Error: {rel_err:.3f}%")
...
if __name__ == "__main__":
    ...
    for p in nonlinearity_powers:
        ...
        if run_comparison:
            ...
            comparison_save_dir = f"comparison_results_p{p}_{potential_type}_with_pretraining_matt"
