...
class GrossPitaevskiiPINN(nn.Module):
    ...
    def __init__(self, layers, hbar=1.0, m=1.0, mode=0, gamma=1.0, potential_type='gravity well'):
        """
        Parameters
        ----------
        layers : list of int
    ...
    def airy_solution(self, x: torch.Tensor, n: int) -> torch.Tensor:
        """
        Gravitational‐well basis Ψₙ(x)=Aₙ·Ai(x + zₙ) for x>=0, zero elsewhere.
        Dimensionless units: ξ = x.
        """
        # Nth zero of Ai(z)=0:
        airy_zeros = [
        ...
        if n < len(airy_zeros):
            ...
            z_n = -((1.5 * np.pi * (n + 0.75)) ** (2.0 / 3.0))

        # 3) prepare output
        psi_np = np.zeros_like(x_np, dtype=np.float32)

        # 4) only for x>=0
        mask = (x_np >= 0)
        if mask.any():
            vals = airy(x_np[mask] + z_n)[0]

            # 5) normalize over x>=0
            dx = float((x[1] - x[0]).item()) if x.numel() > 1 else 0.01
            norm = np.sqrt(np.sum(vals**2) * dx)
            if norm > 1e-12:
                vals /= norm

            psi_np[mask] = vals

        # 6) back to torch, same device & dtype
        return torch.tensor(psi_np).to(device=x.device, dtype=x.dtype)
    ...
    def compute_potential(self, x, potential_type="gravity well", **kwargs):
        """
        V(x) = x for x>=0, ≈∞ for x<0 (hard wall).
        """
        return x
        

    def pde_loss(self, inputs, predictions, gamma, potential_type='gravity well', precomputed_potential=None):
        """
...
def train_gpe_model(gamma_values, modes, X_train, lb, ub, layers, 
                    ...
                    potential_type='gravity well', lr=1e-3, verbose=True):
    """
    ...
    for mode in modes:
        ...
        for gamma in gamma_values:
            ...
            model = GrossPitaevskiiPINN(layers, mode=mode, gamma=gamma, potential_type=potential_type).to(device)
            ...
            # Cr teate schedulero decrease learning rate during training
...
def plot_wavefunction(models_by_mode, X_test, gamma_values,
    ...
    for mode in modes:
        ...
        plt.xlim(0, 20)  # Match paper's range
        plt.tight_layout()
...
if __name__ == "__main__":
    ...
    save_dir = f"plots_matthew_gravity_well"
    os.makedirs(save_dir, exist_ok=True)

    # Gamma values from the paper
    gamma_values = [k*(0.5) for k in range(5)]
    ...
    models_by_mode, mu_table, training_history, constant_history = train_gpe_model(
        gamma_values, modes, X, lb, ub, layers, epochs, tol=0.00001, perturb_const=0, 
        potential_type='gravity well', lr=1e-2, verbose=True )
    print("Training completed!")
    print(constant_history[0])

    # Plot wavefunctions (not densities) for individual modes
    print("Generating individual mode plots...")
    plot_wavefunction(models_by_mode, X_test, gamma_values, modes, constant_history, perturb_const=0, save_dir=save_dir)

    # Plot μ vs γ for all modes
    print("Generating chemical potential vs. gamma plot...")
    plot_mu_vs_gamma(mu_table, modes, save_dir=save_dir)
    ...
    plot_improved_loss_visualization(training_history, modes, gamma_values, epochs, save_dir=save_dir)
    plot_all_modes_gamma_loss(training_history, modes, gamma_values, epochs, save_dir=save_dir)