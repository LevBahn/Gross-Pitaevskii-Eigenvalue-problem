    ...
    for mode in modes:
        ...
        for gamma in gamma_values:
            ...
            for epoch in range(epochs):
                ...
                if count == 2000:
                    print(f"Training is stopped as there was no improvement for last 1000 epochs with error {Loss_criteria}")
                    break
                ...
                if total_loss <= tol:
                    print(f"Early Stop as we have small enough error : {total_loss} with {epoch} epochs")
                    break
...
if __name__ == "__main__":
    ...
    #gamma_values = [k*(0.5) for k in range(201)]
    alpha = 0.5
    ...
    models_by_mode, mu_table, training_history, constant_history = train_gpe_model(
        gamma_values, modes, X, lb, ub, layers, epochs, tol, perturb_const, 
        potential_type='harmonic', lr=1e-3, verbose=True )
    print("Training completed!")

    save_dir = f"plots_matthew"
    os.makedirs(save_dir, exist_ok=True)

    # Plot wavefunctions for individual modes
    print("Generating individual mode plots...")
    plot_wavefunction(models_by_mode, X_test, gamma_values, modes, constant_history, perturb_const, save_dir=save_dir)
    ...
    plot_improved_loss_visualization(training_history, modes, gamma_values, epochs, p=3, potential_type="harmonic", save_dir=save_dir)
