...
def train_regular_pinn(mode, gamma, p, X_train, lb, ub, layers, epochs, lr=1e-3, tol=1e-5, perturb_const=0.01,
    ...
    for epoch in range(epochs):
        ...
        if current_loss <= tol or patience_counter >= patience:
            if verbose:
                print(f"Regular PINN - Mode {mode}, Î³={gamma}, stopped at epoch {epoch}, loss: {current_loss:.6f}")
            break

        if verbose and epoch % 500 == 0:
            print(f"Regular PINN - Mode {mode}, Î³={gamma}, Epoch {epoch}, Loss: {current_loss:.6f}")
...
def train_or_load_regular_pinns(modes, gamma_values, p, X_train, lb, ub, layers, epochs,
    ...
    if not force_retrain:
        ...
        if (regular_models is not None and
            ...
            print("âœ“ Found matching cached Regular PINN models!")
            return regular_models, regular_constants
        else:
            print("âœ— Cached Regular PINN models don't match current parameters.")
    ...
    for mode in modes:
        regular_models[mode] = {}
        for gamma in gamma_values:
            print(f"Training Regular PINN - Mode {mode}, Î³={gamma}")
    ...
    for mode in modes:
        for gamma in comparison_gammas:
            if mode in regular_models and gamma in regular_models[mode]:
                ...
                print(f"Regular PINN - Mode {mode}, Î³={gamma} -> Abs Error: {abs_err:.2e}, Rel Error: {rel_err:.3f}%")
...
if __name__ == "__main__":
    ...
    for p in nonlinearity_powers:
        ...
        filename = f"my_gpe_models_p{p}_{potential_type}_negative_interaction_strength_pert_const_1e-2_tol_{tol}.pkl"

        # Create plotting and model saving directory
        p_save_dir = f"plots_p{p}_{potential_type}_negative_interaction_strength_paper_test"
