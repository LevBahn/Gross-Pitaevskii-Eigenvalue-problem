    ...
    for mode in modes:
        ...
        for mu in mu_values:
            ...
            for epoch in range(epochs):
                ...
                # Total loss with weighted components following the paper's approach
                total_loss = (
                        weights[0] * pde_loss / (ub - lb) +  # Scale PDE loss
                        weights[1] * norm_loss +  # Normalization constraint
                        weights[2] * sym_loss +  # Symmetry constraint
                        weights[3] * boundary_loss  # Boundary conditions
                )
...
def plot_figure11_corrected(models_by_mode, X_test, modes=[0, 1, 2, 3], save_dir="plots"):
    ...
    -----------
    models_by_mode : dict
        Dictionary containing trained models for each mode and mu value
    ...
    # Different line styles for different mu values
    linestyles = ['-', '--', '-.', ':', '-', '--']
    colors = ['k', 'b', 'r', 'g', 'm', 'c']
    ...
    for idx, mode in enumerate(modes):
        ...
        # Get the mu values for this mode from the model dictionary
        ...
        for i, mu in enumerate(mu_values):
            ...
            with torch.no_grad():
                ...
                # Normalize the wavefunction properly
                u_np = full_u.cpu().numpy().flatten()
                u_np /= np.sqrt(np.sum(u_np ** 2) * dx)

                # Plot the wavefunction ψ(x), NOT |ψ(x)|²
        ...
        ax.set_ylabel("ψ(x)", fontsize=12)  # Correct y-axis label
        ax.grid(True)
        ax.legend()
